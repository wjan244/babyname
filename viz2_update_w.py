...
# Fusionner prof (qui contient part_z) avec counts_map pour enrichir les properties
counts_map_enriched = counts_map.merge(
    prof[["preusuel", "periode", "region", "part_z"]],
    on=["preusuel", "periode", "region"],
    how="left"
)
...
features_enrichis = [
    {
        ...
        "properties": {
            ...
            "part": float(row["part"]),
            "part_z": float(row["part_z"]),
        },
    }
    for _, row in counts_map_enriched.iterrows()
...
map_chart = (
    ...
    .encode(
        color=alt.Color("properties.part_z:Q",
                        scale=alt.Scale(domain=[-yabs, yabs], scheme="redblue"),
                        title="Niveau de disparité"),
        tooltip=["properties.nom:N", 
                 alt.Tooltip("properties.part:Q", format=".2%"),
                 alt.Tooltip("properties.part_z:Q", format=".2f")],
    )
    .project(type="mercator")
    .properties(width=450, height=450, title="Disparité par région")
